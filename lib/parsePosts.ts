...
export function getSortedPostsData() {
  ...
  const allPostsData = fileNames.map((fileName) => {
    ...
    return { id, ...(data as ParsedMD) }; // combine metadata with the id
  });
...
export async function getPostData(id: string) {
  ...
  let parsedContent = await remark().use(html).process(content); // parse markdown content to html
  let contentStr = parsedContent.toString(); // parse html to string

  return { id, content: contentStr, ...data };
}
